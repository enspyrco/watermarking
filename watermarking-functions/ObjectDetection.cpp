...
// also: http://stackoverflow.com/a/27533437/1992736 (for recent changes to
// using SURF)
...
int detectObject(Mat& img_object, Mat& img_scene, Mat& detected_img) {
  try {
    ...
    // use the inverse perspective transform to extract the object image from the
    // scene
...
void detectKeypoints(cv::Ptr<cv::Feature2D> f2d, cv::Mat& img,
...
void computeDescriptors(cv::Ptr<cv::Feature2D> f2d, cv::Mat& img, std::vector<KeyPoint> keypoints,
                        cv::Mat& descriptors) {
  try {
...
void calculateGoodMatches(cv::Mat& img_object, cv::Mat& img_scene, cv::Mat& descriptors_object,
                          cv::Mat& descriptors_scene, std::vector<DMatch>& good_matches) {
  try {
    //-- Step 3: Matching descriptor vectors using FLANN matcher
...
void calculateGoodMatchesWithBF(cv::Mat& img_object, cv::Mat& img_scene,
                                cv::Mat& descriptors_object, cv::Mat& descriptors_scene,
...
void calculateHomography(std::vector<KeyPoint> keypoints_object,
                         std::vector<KeyPoint> keypoints_scene,
                         std::vector<cv::DMatch> good_matches, Mat& H) {
  //-- Localize the object
  std::vector<Point2f> obj;
  std::vector<Point2f> scene;
...
// takes the scene image and the original object image and uses the transform to
// draw lines around the object in the scene
// - the original object is only used to find the corners, which are transformed
// by the homograhy
...
void detectWatermark(double pixelsArray[], int pixelsHeight, int pixelsWidth,
                     double watermarkArray[], int watermarkHeight, int watermarkWidth,
                     double correlationVals[]) {
  double* extracted_mark = new double[watermarkHeight * watermarkWidth];

  extractMark(pixelsHeight, pixelsWidth, watermarkHeight, watermarkWidth, pixelsArray,
              extracted_mark);
