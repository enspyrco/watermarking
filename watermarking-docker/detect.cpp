...
int main(int argc, const char * argv[]) {
    ...
    // args are: unique id for db entry, file path for original image, file path for marked image
    if(argc != 4) {
    ...
    // set variables for the image rows and cols, now we know original and marked are the same size 
    ...
    for(int y = 0; y < imgRows; y++)
    {
        ...
        {
            lumaArray[y*imgCols + x] = (hsvMarked.at<cv::Vec3b>(y,x).val[2] - hsvOriginal.at<cv::Vec3b>(y,x).val[2]) / 255.0;
        }
    }
    ...
    while(1) {
        ...
        // calculate peak2rms 
        ms = 0;
        for (int i = 0; i < p*p; i++) ms += (correlationVals[i] * correlationVals[i]) / (p*p);
        ...
        if(peak2rms > 6) {
            ...
            shifts.push_back(maxY*p+maxX); // store the detected shift (the shift that had the peak value) 
            psnrs.push_back(peak2rms); 

        }
        else {

            // TODO: if keeping the last peak, it will need to be removed before being used in getASCII 

            // add the last peak as well, so that results include the last rejected peak  
            // shifts.push_back(maxY*p+maxX); 
            // psnrs.push_back(peak2rms); 
